import { Percent, Price, sqrt, Token, CurrencyAmount, TradeType, WETH9, Ether, Currency } from '@taraswap/sdk-core'
import { Pair } from '@taraswap/v2-sdk'
import { encodeSqrtRatioX96, FeeAmount, nearestUsableTick, Pool, TickMath, TICK_SPACINGS } from '@taraswap/v3-sdk'
...
describe('MixedRouteTrade', () => {
  ...
  const pool_weth_0 = v2StylePool(
    CurrencyAmount.fromRawAmount(WETH9[841], JSBI.BigInt(100000)),
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WETH9[841], JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000))
  )
    ...
    describe('#fromRoute', () => {
      it('can be constructed with ETHER as input', async () => {
        const trade = await MixedRouteTrade.fromRoute(
          ...
          CurrencyAmount.fromRawAmount(Ether.onChain(841), JSBI.BigInt(10000)),
    ...
    describe('#fromRoutes', () => {
      it('can be constructed with ETHER as input with multiple routes', async () => {
        const trade = await MixedRouteTrade.fromRoutes<Ether, Token, TradeType>(
          [
            {
              amount: CurrencyAmount.fromRawAmount(Ether.onChain(841), JSBI.BigInt(10000)),
    ...
    describe('#bestTradeExactIn', () => {
      ...
      it('works for ETHER currency input', async () => {
        const result = await MixedRouteTrade.bestTradeExactIn(
          ...
          CurrencyAmount.fromRawAmount(Ether.onChain(841), JSBI.BigInt(100)),
          token3
        )
        expect(result).toHaveLength(2)
        expect(result[0].inputAmount.currency).toEqual(ETHER)
        expect(result[0].swaps[0].route.path).toEqual([WETH9[841], token0, token1, token3])
        expect(result[0].outputAmount.currency).toEqual(token3)
        expect(result[1].inputAmount.currency).toEqual(ETHER)
        expect(result[1].swaps[0].route.path).toEqual([WETH9[841], token0, token3])
      ...
      it('works for ETHER currency output', async () => {
        ...
        expect(result[0].swaps[0].route.path).toEqual([token3, token0, WETH9[841]])
        expect(result[0].outputAmount.currency).toEqual(ETHER)
        expect(result[1].inputAmount.currency).toEqual(token3)
        expect(result[1].swaps[0].route.path).toEqual([token3, token1, token0, WETH9[841]])
