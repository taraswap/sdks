...
import { Trade as V2Trade, Pair } from '@taraswap/v2-sdk'
import { Trade as V3Trade, Pool, encodeRouteToPath } from '@taraswap/v3-sdk'
import {
...
import { Currency, TradeType, CurrencyAmount, Percent } from '@taraswap/sdk-core'
...
interface Swap<TInput extends Currency, TOutput extends Currency> {
  route: IRoute<TInput, TOutput, Pool | Pair>
...
function addMixedSwap<TInput extends Currency, TOutput extends Currency>(
  ...
  const trade = MixedRouteTrade.createUncheckedTrade({
    route: route as unknown as MixedRoute<TInput, TOutput>,
    inputAmount,
    outputAmount,
    tradeType,
  })
  ...
  const sections = partitionMixedRouteByProtocol(route as unknown as MixedRoute<TInput, TOutput>)
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (mixedRouteIsAllV3(newRoute)) {
      ...
      planner.addCommand(CommandType.V3_SWAP_EXACT_IN, [
        ...
        isLastSectionInRoute(i) ? tradeRecipient : (sections[i + 1][0] as unknown as Pair).liquidityToken.address,
